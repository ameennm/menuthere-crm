...
export default function Reminders({ showToast, preFilter = "all", onRefresh }) {
    ...
    const filtered =
        filterType === "all" ? scheduled : scheduled.filter((c) => c._kind === filterType);
            ...
            <div className="page-content">
                ...
                {filtered.length === 0 && (
                    <div className="empty-state">
                        <Bell size={48} />
                        <h4>No reminders {filterType !== "all" ? `in "${filterType}"` : ""}</h4>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    {filtered.map((c) => {
                        ...
                        const hasTime = c.nextCallDate.includes('T') && !c.nextCallDate.endsWith('T00:00');
                        const formattedDate = callDate.toLocaleDateString("en-IN", {
                            weekday: "short", day: "numeric", month: "short", year: "numeric",
                        });
                        const formattedTime = hasTime ? callDate.toLocaleTimeString("en-IN", {
                        ...
                        return (
                            <div
                                ...
                                style={{
                                    padding: "16px 20px",
                                    borderLeft: `4px solid ${c._kind === "overdue" ? "var(--accent-red)" :
                                        c._kind === "today" ? "#f59e0b" :
                                            c._kind === "soon" ? "#f97316" :
                                                "var(--accent-green)"
                                        }`,
                                    display: "flex",
                            ...
                            >
                                ...
                                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                        <div className="cc-avatar" style={{ width: 38, height: 38, fontSize: 15, flexShrink: 0 }}>
                                    ...
                                    <span className={`badge ${kind.cls}`} style={{ fontSize: 12 }}>
                                ...
                                <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
                                    <CalendarClock size={15} style={{ marginTop: 2, flexShrink: 0, color: "var(--text-secondary)" }} />
                                    <div>
                                        <div style={{ fontWeight: 600, fontSize: 13 }}>
                                            {formattedDate}
                                            {formattedTime && (
                                                <span style={{ marginLeft: 8, fontWeight: 500, color: "var(--accent-blue, #60a5fa)", fontSize: 12 }}>
                                                    ⏰ {formattedTime}
                                                </span>
                                            )}
                                        </div>
                    ...
                    <div className="modal" style={{ maxWidth: 360 }} onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-body">
                            <p style={{ marginBottom: 12, color: "var(--text-secondary)", fontSize: 14 }}>
                                Rescheduling call with <strong style={{ color: "var(--text-primary)" }}>{rescheduleTarget.name}</strong>
                            </p>
                        ...
                        <div className="modal-footer">
                            <button className="btn btn-secondary" onClick={() => setRescheduleTarget(null)}>
                                Cancel
                            </button>
                            <button className="btn btn-primary" onClick={handleReschedule} disabled={!newDate}>
                                Confirm
                            </button>
                        </div>
                    </div>
